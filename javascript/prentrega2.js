document.addEventListener('DOMContentLoaded', () => {
    ...
    const validateEmail = (email) => {
        const apiUrl = `https://www.disify.com/api/email/${email}`;

        fetch(apiUrl)
            .then(response => response.json())
            .then(data => {
                if (data.result === "valid") {
                    // Email es válido, proceder con el registro
                    console.log("Email válido");
                    registerForm.submit(); // Aquí el formulario se enviaría si el correo es válido
                } else {
                    // El email no es válido, mostrar alerta
                    alert("El email ingresado no es válido. Por favor, ingresa un correo electrónico correcto.");
                }
            })
            .catch(error => {
